...
→ v7의 `react-router`은 v6의 선언적 API(`BrowserRouter`, `Routes`, `Route` 등)를 100% 하위 호환 재노출합니다. 이 실습에서는 **URL ↔ 컴포넌트 매핑**이라는 핵심 개념을 흐리지 않기 위해 v6 스타일로 배웁니다.

**Q. import는 어떻게 써야 하나요?**
→ 모든 실습에서 `react-router`에서 import합니다.
